...
- **(R)each →** Quante persone o utenti avranno un beneficio lavorando su questa idea? Si parte da un **rapporto sulla popolazione rilevante per quella Product Line** (non un valore assoluto) — solo così i punteggi restano confrontabili tra Product Line diverse nello stesso Backlog Refinement. Esempi di rapporto per Product Line: employee coinvolti / employee totali (Conversion); ore di backoffice risparmiate / ore totali disponibili di backoffice (Internals); benefit coinvolti / benefit totali offerti (Merchant); prospect toccati / prospect totali B2B nel periodo (WEB). Quel rapporto si porta poi sulla **scala 1-10** come gli altri tre fattori: `reach_points = max(1, ceil(percentuale / 10))` — arrotondamento per eccesso, minimo 1 (es. 15% → 2, 3% → 1, 50% → 5). Il valore assoluto e la percentuale restano come nota di contesto nel record dell'idea, non entrano nello score.
